  ...
  template: `
    ...
    @if (viewMode === 'grid') {
      ...
      >
        ...
        >
          <div class="flex items-center gap-2.5 min-w-0">
            ...
            <div class="flex flex-col min-w-0">
              <h3
                class="font-semibold text-sm leading-none text-foreground flex items-center gap-1 truncate"
              >
              ...
              <p class="text-[10px] text-muted-foreground mt-0.5">
                Added {{ timeAgo(item.last_scraped_at) }}
              </p>
            </div>
          </div>
          ...
          <div class="relative action-menu-container">
            <button
              ...
              class="p-1.5 rounded-full hover:bg-muted text-muted-foreground hover:text-foreground transition-all"
              title="Options"
            >
            ...
            @if (showActionsMenu()) {
              <div
                class="absolute right-0 mt-1 w-36 bg-card border border-border rounded-lg shadow-card-hover py-1 z-20 text-sm"
              >
                <button
                  (click)="onEdit($event)"
        ...
        <!-- Image -->
        <div
          class="relative w-full aspect-square bg-muted/30 flex items-center justify-center border-b border-border/40 overflow-hidden"
        >
          @if (item.image_url && !hasImgError()) {
            ...
            <svg
              class="w-12 h-12 text-muted-foreground/20"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          <div class="absolute top-2 left-2 flex flex-col gap-1 z-10">
            @if (priceDrop > 0) {
              <div
                class="bg-primary text-primary-foreground text-[10px] font-bold px-2 py-0.5 rounded-md"
              >
                -{{ dropPercent }}% OFF
              </div>
            }
            @if (isTargetReached) {
              <div
                class="bg-emerald-600 text-white text-[10px] font-bold px-2 py-0.5 rounded-md"
              >
                TARGET MET
              </div>
            }
          ...
          @if (item.is_purchased) {
            <div
              class="absolute inset-0 bg-background/85 backdrop-blur-[2px] flex items-center justify-center z-10"
            >
              <div class="text-center">
                <svg
                  class="w-10 h-10 mx-auto mb-1 text-emerald-600"
                  fill="none"
                  stroke="currentColor"
                  stroke-width="2"
                  viewBox="0 0 24 24"
            ...
            <div class="flex flex-col gap-0.5">
              <div class="flex items-baseline gap-2 flex-wrap">
                @if (item.current_price !== null) {
                  <span class="font-bold text-lg text-foreground"
                    >₹{{ item.current_price | number: '1.0-0' }}</span
                  >
                } @else {
                ...
                @if (
                  ...
                  @if (priceDirection === 'down') {
                    <span
                      class="inline-flex items-center text-xs font-semibold text-emerald-500 bg-emerald-500/10 px-1.5 py-0.5 rounded gap-0.5"
                    >
                      <svg
                    ...
                    <span
                      class="inline-flex items-center text-xs font-semibold text-rose-500 bg-rose-500/10 px-1.5 py-0.5 rounded gap-0.5"
                    >
                      <svg
              ...
              @if (item.initial_price !== null) {
                <span class="text-[10px] text-muted-foreground"
                  >Created price: ₹{{
                    item.initial_price | number: '1.0-0'
                  }}</span
                >
              }
            </div>
              ...
              @if (item.target_price) {
                <span
                  ...
                  <span class="font-medium text-foreground/80"
                    >₹{{ item.target_price | number: '1.0-0' }}</span
                  ></span
                >
              }
            ...
            <div class="flex gap-1.5 flex-wrap my-3">
              @for (tag of item.tags; track tag) {
                <span
                  ...
                  class="text-primary text-[10px] font-medium bg-primary/5 px-2 py-0.5 rounded-full hover:bg-primary/10 cursor-pointer"
                >
                  #{{ tag }}
                </span>
              }
      ...
      >
        ...
        <div class="flex-1 min-w-0 py-0.5">
          <div class="flex items-center gap-2">
            ...
            @if (priceDrop > 0) {
              <span
                class="bg-primary/10 text-primary text-[9px] font-bold px-1.5 py-0.5 rounded shrink-0"
              >
                -{{ dropPercent }}%
              </span>
            }
          ...
          <div class="flex items-baseline gap-2 mt-1 flex-wrap">
            ...
            @if (
              ...
              @if (priceDirection === 'down') {
                <span
                  class="inline-flex items-center text-[10px] font-semibold text-emerald-500 bg-emerald-500/10 px-1 py-0.2 rounded gap-0.5"
                >
                  <svg
                ...
                <span
                  class="inline-flex items-center text-[10px] font-semibold text-rose-500 bg-rose-500/10 px-1 py-0.2 rounded gap-0.5"
                >
                  <svg
                    class="w-2.5 h-2.5 text-rose-500"
                    fill="none"
                    stroke="currentColor"
                    stroke-width="2.5"
                    viewBox="0 0 24 24"
            ...
            @if (item.initial_price !== null) {
              <span class="text-xs text-muted-foreground"
                >(Created: ₹{{ item.initial_price | number: '1.0-0' }})</span
              >
            }
            @if (item.target_price) {
              <span class="text-xs text-muted-foreground"
                >(Target: ₹{{ item.target_price | number: '1.0-0' }})</span
              >
            }
          </div>
          ...
          @if (item.tags?.length) {
            <div class="flex gap-1 flex-wrap mt-1">
              @for (tag of item.tags; track tag) {
                <span
                  (click)="onTagClick($event, tag)"
                  class="text-primary text-[9px] font-medium bg-primary/5 px-1.5 py-0.2 rounded-full cursor-pointer hover:bg-primary/10"
                >
                  #{{ tag }}
                </span>
              }
        ...
        <div class="flex items-center gap-2 shrink-0">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            class="p-2 rounded-full hover:bg-muted text-muted-foreground hover:text-foreground transition-all hidden xs:flex"
            title="Buy Now"
          >
            <svg
              class="w-5 h-5"
            ...
            @if (showActionsMenu()) {
              ...
              >
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  class="flex xs:hidden items-center w-full px-3 py-2 hover:bg-muted text-foreground transition-colors gap-2"
                >
                  <svg
                    class="w-3.5 h-3.5 text-muted-foreground"
                    fill="none"
...
export class ItemCardComponent {
  ...
  get priceDrop(): number {
    return this.wishlistSvc.getPriceDrop(this.item);
  }
  get dropPercent(): number {
    return this.wishlistSvc.getPriceDropPercent(this.item);
  }
  get isTargetReached(): boolean {
    return this.wishlistSvc.isTargetReached(this.item);
  }

  get priceDifference(): number | null {
    if (this.item.current_price === null || this.item.initial_price === null)
      return null;
  ...
  async onRegenerateShare(e: MouseEvent) {
    e.stopPropagation();
    this.sharing.set(true);
    const { token, error } = await this.shareSvc.regenerateItemShareToken(
      this.item.id,
    );
    this.sharing.set(false);
    this.showActionsMenu.set(false);
